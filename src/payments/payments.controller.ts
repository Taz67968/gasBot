...
export class PaymentsController {
  ...
  async confirmCashByAgent(
    ...
    this.logger.log(
      `Agent ${agentId} requesting cash confirmation for order ${dto.orderId}`,
    );

    try {
      const result = await this.paymentsService.confirmCashCollectionByAgent(
        dto.orderId,
        agentId,
      );

      this.logger.log(
        `Cash collection confirmed by agent ${agentId} for order ${dto.orderId}`,
      );
      return result;
    } catch (err: any) {
      this.logger.error(
        `Failed to confirm cash for order ${dto.orderId}: ${err.message}`,
      );
      throw err;
    }
  }
