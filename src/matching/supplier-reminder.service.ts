...
export class SupplierReminderService implements OnModuleInit {
  ...
  async onModuleInit() {
    this.logger.log('Supplier reminder service initialized — sending first batch immediately');
  ...
  private async sendReminders(): Promise<void> {
    try {
      ...
      const text = 'Hi! Stay ready for orders: please reply to this message with "OK" to keep receiving new gas order notifications.';
      ...
      for (const agent of agentRows) {
        ...
        try {
          ...
          this.logger.log(`Session reminder sent to agent ${agent.id} (${agent.full_name}) at ${phone}`);
        } catch (err: unknown) {
          const message = err instanceof Error ? err.message : 'Unknown error';
          this.logger.warn(`Failed to send reminder to ${phone}: ${message}`);
        }
      }
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : 'Unknown error';
      this.logger.error(`Failed to run supplier reminders: ${message}`);
    }
  }
}
